...
The images below were generated by the real `render_ui` pipeline from the JSON specs that follow them.

### Metric cards row

Light and dark themes use the same spec with only `theme` changed.

![Metric cards row, light theme](docs/images/metric-row-light.png)
![Metric cards row, dark theme](docs/images/metric-row-dark.png)

### Bar chart + donut chart in plain boxes

Charts render as crisp SVG geometry with token-driven overlays for labels and legends.

![Bar chart and donut chart, light theme](docs/images/charts-box-light.png)
![Bar chart and donut chart, dark theme](docs/images/charts-box-dark.png)

### Table + alerts composition

Tables and callouts share the same color, spacing, and radius tokens, so they look consistent across both themes.

![Table and alerts, light theme](docs/images/table-alert-light.png)
![Table and alerts, dark theme](docs/images/table-alert-dark.png)

### Full dashboard exemplar

A richer layout using `examples/dashboard.json` with metrics, a line chart, a progress ring, a table, and alerts.

![Dashboard exemplar, light theme](docs/images/dashboard-light.png)
![Dashboard exemplar, dark theme](docs/images/dashboard-dark.png)
...
Invalid specs are returned as structured errors with this shape:

```json
{
  "code": "VALIDATION_ERROR",
  "path": ".elements.body.props.data",
  "message": "..."
}
```

Unexpected renderer failures use the same contract with `code: "RENDER_ERROR"` and `path: "."`.

The spec is bounded for untrusted MCP input:

| Limit | Value | What it protects |
|---|---|---|
| Max elements | 2,000 | Total keys in `elements` |
| Max tree depth | 50 | Element reference depth from root |
| Max string length | 10,000 characters | Any string prop or nested string |
| Max array length | 1,000 entries | Any array anywhere in the spec |
| Max chart points | 1,000 points | Per chart series or sparkline |

Cross-check the live catalog and prop schemas with the `list_components` tool rather than relying on a static list.
...
Representative benchmark on the included `examples/dashboard.json` exemplar: well under 5 seconds and under 2 MB on a typical workstation, rendered at the default 2× density.

## Known limitations

- **No runtime network access.** `Avatar` in `mode: "image"` expects a base64 `data:` URI; remote URLs are not fetched.
- **Charts need explicit sizing inside flexible parents.** A chart placed directly inside a `Card` or `Row` may collapse unless it has an explicit `width`/`height` or its parent provides a definite cross-axis size. Place charts inside `Box` containers with explicit sizing for the most predictable results, as shown in the Examples section above.
